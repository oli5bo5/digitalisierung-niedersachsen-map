    ...
    return (
      <div className="w-full h-screen flex items-center justify-center bg-red-50">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-red-900 mb-4">Mapbox Token nicht konfiguriert</h1>
          <p className="text-red-700">Bitte stellen Sie sicher, dass NEXT_PUBLIC_MAPBOX_TOKEN in den Umgebungsvariablen gesetzt ist.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-screen flex flex-col">
      <div className="bg-gradient-to-r from-indigo-900 to-indigo-700 text-white p-6 shadow-lg">
        <h1 className="text-3xl font-bold mb-2">Interaktive Karte der Digitalisierungsakteure</h1>
        <p className="text-indigo-100">Niedersachsen - Entdecken Sie die Digitalisierer in Ihrer Region</p>
      </div>
      ...
      <div className="flex-1 flex gap-4 p-4 overflow-hidden bg-gray-100">
        <div className="w-80 bg-white rounded-lg shadow-lg p-6 overflow-y-auto flex flex-col">
          <h2 className="text-xl font-bold text-indigo-900 mb-4">Regionen in Niedersachsen</h2>
          <div className="space-y-2 mb-6">
            {regions.map((region) => (
              <button
                key={region.id}
                onClick={() => setViewState({longitude: region.lng, latitude: region.lat, zoom: 10})}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (region) {
                      setFormData({...formData, region: e.target.value, latitude: region.lat, longitude: region.lng});
                    }
                  }}
                  className="w-full px-3 py-2 border border-indigo-300 rounded text-sm"
                >
          ...
          <div className="border-t pt-4 flex-1 overflow-y-auto">
            <h3 className="text-lg font-bold text-indigo-900 mb-3">Akteure ({stakeholders.length})</h3>
            {loading ? (
              ...
              <div className="space-y-2">
                {stakeholders.map((s) => (
                  <button
                    ...
                    onClick={() => {
                      setSelectedMarker(s);
                      setViewState({longitude: s.longitude, latitude: s.latitude, zoom: 12});
                    }}
          ...
          >
            ...
            {regions.map((region) => (
              <Marker key={region.id} longitude={region.lng} latitude={region.lat}>
                <div className="cursor-pointer font-bold text-white bg-indigo-600 rounded-full w-10 h-10 flex items-center justify-center shadow-lg hover:bg-indigo-700 text-sm" title={region.name}>
                  ◉
                </div>
              </Marker>
            ))}
            ...
            {selectedMarker && (
              ...
              >
                <div className="p-3 bg-white rounded">
                  ...
                  {selectedMarker.description && <p className="text-xs text-gray-600 mt-2">{selectedMarker.description}</p>}
                </div>
              </Popup>
            )}
          </Map>
